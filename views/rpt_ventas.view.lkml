...
FROM
  `@{GCP_PROJECT}.@{REPORTING_DATASET}.rpt_ventas` v LEFT JOIN
  `@{GCP_PROJECT}.@{REPORTING_DATASET}.CALENDAR` c on v.CALDAY=c.CALDAY LEFT JOIN
    `@{GCP_PROJECT}.@{REPORTING_DATASET}.REGIONES` r ON v.SALESORG = r.SALESORGANIZATION
...
SELECT
    ...
    V.ZIOSD00A, V.VERSION, V.PLANT, V.MATERIAL,
    V.DISTR_CHAN, V.SALESORG, V.CALDAY, V.BASE_UOM,
    V.CATEGORY, '0' AS SUBCATEGORY, V.CLIENT, CAST(c.DATE AS TIMESTAMP) Fecha,
...
FROM
    `@{GCP_PROJECT}.@{REPORTING_DATASET}.rpt_ventas` v LEFT JOIN
    `@{GCP_PROJECT}.@{REPORTING_DATASET}.CALENDAR` c on v.CALDAY=c.CALDAY
WHERE
...
FROM
    ...
    (
        ...
        FROM
            `@{GCP_PROJECT}.@{REPORTING_DATASET}.tcurr` left join
            `@{GCP_PROJECT}.@{REPORTING_DATASET}.CALENDAR` c on c.CALDAY=CAST(99999999 - CAST(GDATU AS NUMERIC) AS STRING)
        WHERE
...
FROM
    `@{GCP_PROJECT}.@{REPORTING_DATASET}.rpt_ventas` v LEFT JOIN
    `@{GCP_PROJECT}.@{REPORTING_DATASET}.CALENDAR` c on v.CALDAY=c.CALDAY LEFT JOIN
    `@{GCP_PROJECT}.@{REPORTING_DATASET}.REGIONES` r ON v.SALESORG = r.SALESORGANIZATION LEFT JOIN
    (
        SELECT
            CAST(99999999 - CAST(GDATU AS NUMERIC) AS STRING) AS CALDAY,
            TRIM(FCURR) FCURR, TRIM(TCURR) TCURR,
